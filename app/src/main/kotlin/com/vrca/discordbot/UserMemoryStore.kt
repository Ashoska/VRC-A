...
object UserMemoryStore {
    ...
    private val NEGATED_FACT = Regex("(?i)^(is|was|are)\\s+(not|no longer)\\b|^(isn'?t|wasn'?t|aren'?t|not|no longer)\\b|^(hasn'?t|haven'?t|has not|have not) been\\b|^(doesn'?t|does not) (live|work)\\b|^(left|quit|stopped|moved (out of|away from|from))\\b|^(has |have )?never\\b")
