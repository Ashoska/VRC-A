...
- **DiscordLoginWebView**: WebView-based Discord login flow. User logs into discord.com; login completion uses a 3-layer verification: (1) URL must contain `discord.com/channels` (NOT `/app` — that's a transient session-check artifact), (2) 2-second dwell timer ensures the URL is stable (cancelled if page navigates away), (3) JS authentication probe checks `webpackChunkdiscord_app` exists AND no login form (`input[type="password"]`, `input[type="email"]`) is visible. The probe retries up to 4 times at 1.5s intervals when it returns false, since Discord's SPA can take a few seconds to hydrate `webpackChunkdiscord_app` after the auto-redirect from `/login` → `/channels/@me` for users with valid cached cookies. All retry runnables are tracked and cancelled in `onPageStarted` so a navigation away from `/channels` (e.g., bouncing back to `/login`) cannot trigger a stale `onLoginComplete`. Only after all 3 layers pass does `onLoginComplete` fire and `discord_session_seeded` is set.
