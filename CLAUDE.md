...
### VRChat Integration
- **VrchatAuthManager**: Singleton handling VRChat API auth (Basic auth + 2FA), cookie storage via EncryptedSharedPreferences, presence fetching, and friends list retrieval. Also saves credentials to EncryptedSharedPreferences for auto-relogin when sessions expire. On re-login after auth-cookie expiry, the persisted `twoFactorAuth` cookie is sent alongside the Basic auth header so VRChat recognizes the trusted device and skips the 2FA prompt — works for up to 30 days from the last successful 2FA verification (`KEY_2FA_COOKIE_STORED_AT` tracks the age; expired cookies are dropped before sending). Also provides REST helpers for offline notification backfill: `fetchPendingNotifications()`, `fetchPendingNotificationsV2()`, `fetchUserGroups()`, `fetchGroupAnnouncement(groupId)`. Trust rank extraction (`extractTrustRankFromTags`) reads the highest `system_trust_*` tag from VRChat's tags array.
...
- **Notification system**: Seven Android notification channels — `vrca_pipeline` (background service, MIN), `vrca_friends_activity` (online/offline/world/status/avatar/rank), `vrca_friend_requests` (incoming requests + new friends + messages), `vrca_friend_removals` (unfriends), `vrca_invites` (world + group invites), `vrca_groups` (announcements/queue/role/etc.), `vrca_connection` (pipeline up/down + sign-in alerts + server alerts). Legacy `vrca_vrchat_events` is deleted on upgrade. Notifications use `setGroup()` keys (`vrca_group_friends`, `vrca_group_social`, `vrca_group_invites`, `vrca_group_groups`) so multiple events stack in the shade with summary notifications. Per-friend cooldowns (60s) on chatty events (location, status, avatar). All 26 toggles live in `VrchatNotificationPrefs` keys + `UserPreferencesRepository` flows. The UI lives in `LegacyPagesAndSettingsPage.kt`'s `NotificationToggleSection` (public, called from `ChatboxScreen.kt`), organized into 5 collapsible sections: Friend list (4 toggles: request, new friend, unfriend, VRChat messages), Friends activity (10 toggles: online/offline/active/location/status/avatar/bio/rename/rank change/vote-to-kick), Invites (2: world/group), Groups (6: announcements/queue/instance/join request/role/other), App and connection (6: app update/admin announcements/connection status/sign-in alerts/VRChat server alerts/gifts). Each section is expandable/collapsible via `AnimatedVisibility` with fade+slide animation.
...
- **DiscordRpcService**: Foreground service running a hidden WebView that loads `discord.com`. Discord's own JavaScript handles the gateway connection (intents, capabilities, X-Super-Properties, TLS, /science telemetry) — all traffic looks like a real Discord web session, making detection effectively impossible. After page load, a JS shim (webpack module finder) locates Discord's Flux dispatcher and exposes `window.VRCA_setActivity(json)` / `window.VRCA_clearActivity()`. The native side pushes activity updates via `evaluateJavascript` whenever `VrchatPipelineState.presenceFlow` emits (1.5s debounce) or every 10s as keep-alive. World thumbnails are squash-proxied via `images.weserv.nl` (`fit=fill`). The user token is never extracted or stored — `DiscordLoginWebView` just seeds the `CookieManager` session. Auto-starts when VRChat pipeline connects if `discord_rpc_enabled` and `discord_session_seeded` are both true. `DiscordExternalAssetResolver` is deleted — Discord's own JS handles image resolution server-side.
  - **Safety system (7 features):**
    ...
    3. **Failure detection + response**: Non-silent — failures update `DiscordRpcState.status` (shared `MutableStateFlow`) and the notification. Session expiry shows a red "Sign in again" button in the UI. Module finder failure shows "needs update" status.
...
- **DiscordLoginWebView**: WebView-based Discord login flow. User logs into discord.com; login completion uses a 3-layer verification: (1) URL must contain `discord.com/channels` (NOT `/app` — that's a transient session-check artifact), (2) 2-second dwell timer ensures the URL is stable (cancelled if page navigates away), (3) JS authentication probe checks `webpackChunkdiscord_app` exists AND no login form (`input[type="password"]`, `input[type="email"]`) is visible. Only after all 3 pass does `onLoginComplete` fire and `discord_session_seeded` is set. This prevents premature closure when stale Discord cookies cause a brief redirect to `/app` before bouncing back to `/login`.
