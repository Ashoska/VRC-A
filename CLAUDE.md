...
- **Echo suppression (`lastSelfSyncFingerprint` + `previousSelfSyncFingerprint`)**: Each self-sync write saves the current fingerprint as `previousSelfSyncFingerprint`, then sets `lastSelfSyncFingerprint` to the new value **before** awaiting the Firestore call. The snapshot listener (`applyRemoteConfig`) computes the same fingerprint from incoming snaps via `computeFingerprintFromSnap` and skips if it matches **either** fingerprint. The two-fingerprint check handles the race where a heartbeat merge triggers a snapshot during the self-sync write window: `lastSelfSyncFingerprint` has already been advanced to the new value, but the snapshot still contains old content matching `previousSelfSyncFingerprint`. Without this, stale heartbeat-triggered snapshots would revert the user's fresh edits. On write failure the previous fingerprint is restored so the next sync loop iteration retries.
