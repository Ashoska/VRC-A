...
**Echo suppression** (per-field `lastSyncedValues` map): Two strategies depending on field type. **Toggles** compare against current local state (`afkEnabled`, etc.) — if remote differs from local, it's an admin edit (apply); if same, nothing to change. **Content fields** (strings, ints) compare against `lastSyncedValues` baseline with fallback to current local state. Both paths update `lastSyncedValues` immediately after every snapshot (whether applied or skipped), so subsequent self-sync echoes are correctly suppressed. The map is also bulk-updated after each successful `performSelfSync` write via `captureStateForSync()`. The first snapshot seeds the baseline via `seedLastSyncedFromSnapshot`.
